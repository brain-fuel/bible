...
- **Licensing (all shipped data PD or CC-BY; verified in Task 0, see `docs/FORMATS-lxx.md`):** LXX text = **Swete 1909, Public Domain** (re-derive from a PD edition; do NOT ship the GPL-3.0 CSV packaging). Lemmas = **openscriptures GreekResources `LxxLemmas`, CC-BY 4.0**. Versification = **STEPBible TVTMS, CC-BY 4.0** + a **CC0** hand-authored supplement. Strong's/lexicon = existing repo `lexicon/grc/` (PD spine). REJECTED (do not use): CATSS/CCAT, eliranwong RLXX, CenterBLC morph, CATSS/Tov alignment — all CC-BY-NC or restricted. Provenance (`src`/`sources`) on every datum.
